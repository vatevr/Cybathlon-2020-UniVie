...
class feature_selector :
    ...
    def __init__(self, features=2, method='linear'):
        """
    ...
    def transform(self, X_csp) :
        ...
        :param X : numpy array of shape bands *  epochs * csp_components
        """
        picks = np.zeros((X_csp.shape[1], X_csp.shape[2]*self.features))
        band_ind = self.bands_
        
        for feature in range(self.features) :
            for component in range(X_csp.shape[2]) :
                picks[:, (feature*X_csp.shape[2])+component] = X_csp[band_ind[feature], :, component]
        return picks
    
    def transform_filter(self, csp_filter) :
        """
        :param csp_filters : numpy array of shape bands *  epochs * csp_components
        """
        #import pdb
        #pdb.set_trace()
        picks = np.zeros((csp_filter.shape[1], csp_filter.shape[2]*self.features))
        band_ind = self.bands_
        
        for feature in range(self.features) :
            for component in range(csp_filter.shape[2]) :
                picks[:, (feature*csp_filter.shape[2])+component] = csp_filter[band_ind[feature], :, component]
        return picks
    
 
    def fit_transform(self, X_csp, y) :
        self.fit(X_csp, y)
        return self.transform(X_csp)
