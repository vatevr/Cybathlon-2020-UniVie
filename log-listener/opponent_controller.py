...
class OpponentController():
    def __init__(self, dependencies, folder_path, eeg_amp):
        ...
        if dependencies.get_config_reader().get_opponent_on('first') == True:
            self.opponent_logs.update({self.opponent_tags[0]: []})
            self.opponents.update({self.opponent_tags[0]: Opponent(self.opponent_tags[0], 'first', dependencies.get_config_reader(), folder_path, eeg_amp)})
        if dependencies.get_config_reader().get_opponent_on('second') == True:
            self.opponent_logs.update({self.opponent_tags[1]: []})
            self.opponents.update({self.opponent_tags[1]: Opponent(self.opponent_tags[1], 'second', dependencies.get_config_reader(), folder_path, eeg_amp)})
        if dependencies.get_config_reader().get_opponent_on('third') == True:
            self.opponent_logs.update({self.opponent_tags[2]: []})
            self.opponents.update({self.opponent_tags[2]: Opponent(self.opponent_tags[2], 'third', dependencies.get_config_reader(), folder_path, eeg_amp)})
